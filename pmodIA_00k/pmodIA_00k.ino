...
void setup()
{
  ...
  sendCommand2ControlRegister(STAND_BY);//Serial.println("STAND BY MODE");
  sendCommand2ControlRegister(INIT_START_FREQ);//Serial.println("INIT_START_FREQ");  
  sendCommand2ControlRegister(START_FREQ_SWEEP);//Serial.println("START_FREQ_SWEEP");
  
  /*for(int ii=0; ii<10; ii++){
      sendCommand2ControlRegister(REPEAT_FREQ);
      I=getImaginaryPart();
      R=getRealPart();
...
*/
  ...
  while(cond){
    ...
    cond=  Wire.read()!= FREQ_SWEEP_COMPLETE && ij<600;
    

    Serial.print(M);Serial.print("  "); Serial.println(ij);
    ij++;
  }*/
  //Serial.println("DONE");
...
//Serial.println(1/M);
      if(abs(M-M0)<100){
        ...
        Serial.println(M);};
      //P=atan(I/R);
      //if(M < 10000 && M>1000){Serial.println(M);};
      //Serial.println(M);
      M0=M;
}
/***************
...
bool waitForValidFreqData(){
  ...
  do{ //ASK IF THERE IS VALID FREQUENCY DATA      
    ...
    ii++;
    }while(BIT_1 != VALID_FREQ_DATA && ii<ii_timeOutLim); //END DO-WHILE LOOP
